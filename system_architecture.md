...
Tài liệu này đặc tả chi tiết kiến trúc kỹ thuật, mô hình luồng dữ liệu, liên lạc tiến trình (IPC) và các giải pháp tích hợp công nghệ đa phương tiện đa nền tảng tính đến phiên bản v1.9 hiện tại.
...
| `synthesize-speech` | `main.js` → `ttsJobOrchestrator.js` | Chạy một TTS job hoàn chỉnh, điều phối streaming/REST/fallback và trả metadata engine thực tế. |
| `cancel-tts-job` | `main.js` | Hủy job của đúng renderer sender bằng `AbortController`. |
| `tts-job-progress` | `main.js` → Renderer | Phát phase, engine, phần trăm và lý do retry/fallback. |
| `select/get/validate/clear-google-credentials` | `main.js` → `googleCredentials.js` | Quản lý đường dẫn service-account và trạng thái validation, không trả JSON/private key về renderer. |
...
`TtsScreen.tsx` chỉ sở hữu cấu hình, preview chunk, tiến trình, cancel và kết quả. Một lần bấm tạo audio gửi đúng một `TtsJobRequest` qua preload; renderer không tự gọi từng chunk và không tự ghép audio.

```mermaid
flowchart LR
    UI["TtsScreen: Stable / Expressive"] -->|"synthesize-speech"| MAIN["Electron Main"]
    MAIN --> ORCH["ttsJobOrchestrator"]
    ORCH --> STREAM["chirpStreamingAdapter"]
    ORCH --> REST["googleRestAdapter"]
    STREAM --> PCM["Attempt-specific PCM sink"]
    REST --> PCM
    PCM --> FINAL["audioAssembler: WAV header or one MP3 encode"]
    CREDS["googleCredentials: path-only store"] --> STREAM
    CREDS --> REST
    CHUNK["shared/ttsChunker"] --> ORCH
```

### 4.1. Điều phối attempt và fallback

*   `ttsJobOrchestrator.js` validate request, chọn giới hạn chunk theo provider và tạo một thư mục/PCM sink riêng cho mỗi attempt.
*   Stable thử `chirp-streaming` hai lần khi có service-account hợp lệ, sau đó chạy lại toàn bộ job bằng `chirp-rest`; nếu Chirp REST lỗi, job chạy lại toàn bộ bằng voice Neural2 tương ứng.
*   Expressive chỉ dùng `gemini-rest` và không fallback sang Stable, tránh thay đổi ý nghĩa lựa chọn của người dùng.
*   Attempt lỗi/hủy luôn đóng sink và xóa thư mục tạm. Chỉ attempt thành công mới được finalization tới `request.outputPath`; vì vậy output không thể chứa PCM trộn từ hai engine.

### 4.2. Adapter, credential và audio boundary

*   `chirpStreamingAdapter.js` gửi một config frame đầu tiên, tuần tự các text frame, tôn trọng backpressure và ghi raw PCM phản hồi theo đúng thứ tự.
*   `googleRestAdapter.js` tạo payload riêng cho Chirp/Neural2/Gemini, retry có backoff cho 429/5xx/network, và bóc `RIFF/WAVE` khỏi mỗi phản hồi `LINEAR16` trước khi append PCM.
*   `googleCredentials.js` lưu duy nhất đường dẫn service-account, validate bằng `listVoices`, tạo access token trong Main process và redact token/private key khỏi lỗi. OAuth REST vẫn hoạt động khi không có service-account.
*   `audioAssembler.js` duy trì mono 24 kHz signed 16-bit PCM. WAV nhận đúng một header; MP3 được encode đúng một lần ở cuối bằng `libmp3lame` 256 kbps. Pipeline TTS không dùng `acrossfade`.

---

## 5. ENGINE TẠO PHỤ ĐỀ TRÍ TUỆ NHÂN TẠO (WHISPER AI ENGINE)
...
## 6. ĐẶC TẢ CẤU TRÚC DỮ LIỆU DỰ ÁN (.SBVPROJ DATA SCHEMA)
